...
class AbstractState(AuditMixin):
    ...
    def _get_fields(self):
        return filter(lambda x: isinstance(x, models.NullBooleanField) or isinstance(x, models.BooleanField), self._meta.fields)
...
class SummaryCompletionState(AbstractState):
    ...
    """
    prescription = models.OneToOneField(Prescription, related_name='pre_state',on_delete=models.CASCADE)
    summary = models.BooleanField(choices=BOOL_CHOICES, default=False)
    context_statement = models.BooleanField(choices=BOOL_CHOICES, default=False)
    ...
    priority_justification = models.BooleanField(choices=BOOL_CHOICES, default=False)
    complexity_analysis = models.BooleanField(choices=BOOL_CHOICES, default=False)
    ...
    def clean_context_map(self):
        ...
        if self.context_map and count < 1:
            ...
            self.save() # why need to call 'save' in clean method
...
class BurnImplementationState(AbstractState):
    ...
    prescription = models.OneToOneField(
        Prescription, related_name='day_state',on_delete=models.CASCADE)
...
class PostBurnChecklist(AuditMixin):
    ...
    completed_on = models.DateField(default=timezone.now, blank=True, null=True)
    completed_by = models.TextField(verbose_name="Action completed by (name)",blank=True, null=True)
        ...
        if self.completed_on is not None:
            ...
            if self.completed_on > datetime.now().date():
                raise ValidationError("This action could not be completed "
                                      "in the future.")
            if (self.prescription.approval_status_modified is None or self.completed_on < self.prescription.approval_status_modified.date()):
                raise ValidationError("This action could not be completed "
    ...
    def clean(self, *args, **kwargs):
        ...
        if self.completed_by and self.completed_on is None:
            raise ValidationError("Please specify when was this action completed.")
...
class BurnClosureState(AbstractState):
    ...
    section has been fully completed.
    """
    prescription = models.OneToOneField(Prescription, related_name='post_state',on_delete=models.CASCADE)
    post_actions = models.NullBooleanField(choices=NULL_CHOICES, default=False)
    evaluation_summary = models.BooleanField(choices=BOOL_CHOICES, default=False)
    evaluation = models.NullBooleanField(choices=NULL_CHOICES, default=False)
    post_ignitions = models.NullBooleanField(choices=NULL_CHOICES, default=False)
    aerial_intensity = models.NullBooleanField(choices=NULL_CHOICES, default=False)
    satellite_intensity = models.NullBooleanField(choices=NULL_CHOICES, default=False)
    other = models.NullBooleanField(choices=NULL_CHOICES, default=False)
    post_burn_checklist = models.BooleanField(choices=BOOL_CHOICES, default=False)
    closure_declaration = models.BooleanField(choices=BOOL_CHOICES, default=False)
    ...
    def clean_post_burn_checklist(self):
        checklists = PostBurnChecklist.objects.filter(prescription=self.prescription, relevant=True)
    ...
    def clean_signage(self):
        documents = self.prescription.document_set
        doc_count = 0
        doc_count += self.prescription.way_set.filter(roadsegment__traffic_diagram__isnull=False).count()
        doc_count += documents.filter(tag__name="Traffic Diagrams").count()
        doc_count += documents.filter(tag__name="Diversion Map").count()
        sign_doc_count = documents.filter(tag__name="Sign Inspection and Surveillance Form").count()
...
class ProposedAction(AuditMixin):
    prescription = models.ForeignKey(Prescription, on_delete=models.PROTECT)
...
class ClosureDeclaration(AuditMixin):
